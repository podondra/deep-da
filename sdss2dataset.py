...
if __name__ == "__main__":
    ...
    f = h5py.File("data/sdss/sdss_dataset.hdf5", "x")

    sdss = mat["sdss"]
    X_all = sdss["X"]
    y_all = sdss["y"][:]
    id_all = sdss["filename"][:]

    idx_all = np.arange(id_all.size)

    id_tr, id_te, idx_tr, idx_te, y_tr, y_te = train_test_split(id_all, idx_all, y_all, test_size=0.1)
    id_tr, id_va, idx_tr, idx_va, y_tr, y_va = train_test_split(id_tr, idx_tr, y_tr, test_size=0.2)

    for name, data in [("id_tr", id_tr), ("id_va", id_va), ("id_te", id_te), ("y_tr", y_tr), ("y_va", y_va), ("y_te", y_te)]:
